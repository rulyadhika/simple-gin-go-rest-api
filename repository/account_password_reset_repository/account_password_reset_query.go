...
const findOneAccountPasswordResetDataByTokenQuery = `SELECT user_id, token, request_time, expiration_time, next_request_available_at FROM accounts_password_resets WHERE token=$1 ORDER BY expiration_time DESC LIMIT 1`
const deleteAccountPasswordResetDataByTokenQuery = `DELETE FROM accounts_password_resets WHERE token=$1`
